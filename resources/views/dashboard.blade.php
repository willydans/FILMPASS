                    ...
                    <div class="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8">
                        <div class="max-w-2xl">
                            ...
                            <p class="text-lg md:text-xl text-gray-200 mb-8 drop-shadow-md">
                                Platform pemesanan tiket bioskop termudah. Temukan film terbaru, pilih kursi, dan nikmati
                                pertunjukannya.
                            </p>
                            ...
                            <form action="{{ route('home') }}" method="GET" class="relative max-w-md">
                                ...
                                <button type="submit"
                                    class="absolute right-2 top-1.5 bg-orange-500 hover:bg-orange-600 text-white p-2 rounded-full transition">
                ...
                <div class="relative z-10 -mt-24 md:-mt-32">
                    ...
                    @if ($featuredFilms->count() > 0)
                        <section class="container mx-auto px-4 sm:px-6 lg:px-8 my-10">
                            ...
                            <div x-data="{ activeSlide: 1, totalSlides: {{ $totalSlides }} }" x-init="setInterval(() => { activeSlide = (activeSlide % totalSlides) + 1 }, 6000)"
                                ...
                                <div class="flex transition-transform duration-700 ease-in-out"
                                    ...
                                    @foreach ($featuredFilms as $film)
                                        <div class="flex-shrink-0 w-full relative">
                                            ...
                                            <div
                                                class="relative bg-slate-900/80 backdrop-blur-md min-h-[400px] flex flex-col md:flex-row items-center p-6 md:p-10 gap-8">
                                                ...
                                                <div class="w-full md:w-2/3 lg:w-3/4 text-left">
                                                    <h3 class="text-3xl md:text-4xl font-bold mb-3 text-white">
                                                        {{ $film->title }}</h3>

                                                    <div
                                                        class="flex flex-wrap items-center gap-3 mb-4 text-sm text-gray-300">
                                                        {{-- Badge Rating --}}
                                                        <span
                                                            class="bg-yellow-500/20 text-yellow-400 border border-yellow-500/50 px-2 py-0.5 rounded">
                                                            ⭐ {{ $film->rating }}
                                                        </span>
                                                        ...
                                                        @if ($film->genre)
                                                            @foreach (explode(', ', $film->genre) as $genre)
                                                                <span
                                                                    class="bg-white/10 px-2 py-0.5 rounded hover:bg-white/20 transition cursor-default">
                                                                    {{ $genre }}
                                                                </span>
                                                            @endforeach
                                                        @endif
                    ...
                    <section class="container mx-auto px-4 sm:px-6 lg:px-8 my-20" id="movies">
                        <div class="flex items-end justify-between mb-8">
                            ...
                            @if (!$search)
                                <a href="#"
                                    class="text-orange-500 hover:text-orange-400 text-sm font-semibold flex items-center">
                        ...
                        @if ($popularFilms->count() > 0)
                            <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-6">
                                @foreach ($popularFilms as $film)
                                    <div
                                        class="group relative bg-slate-800 rounded-xl overflow-hidden shadow-lg hover:shadow-2xl hover:shadow-orange-500/10 transition-all duration-300 border border-slate-700 hover:border-slate-600 hover:-translate-y-1">
                                        ...
                                        <div class="relative overflow-hidden aspect-[2/3]">
                                            ...
                                            {{-- Overlay saat Hover --}}
                                            <div
                                                class="absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4">
                                            ...
                                            <div
                                                class="absolute top-2 right-2 bg-black/60 backdrop-blur-md text-yellow-400 text-xs font-bold px-2 py-1 rounded flex items-center">
                                        ...
                                        {{-- Card Content --}}
                                        <div class="p-4">
                                            <h4 class="text-white text-base font-semibold line-clamp-1 mb-1"
                                                title="{{ $film->title }}">
                                                {{ $film->title }}
                                            </h4>
                        ...
                        @else
                            <div class="text-center py-20 bg-slate-800/50 rounded-xl border border-dashed border-slate-700">
                                ...
                                @if ($search)
                                    <a href="{{ route('home') }}"
                                        class="text-orange-500 hover:underline mt-2 inline-block">Reset Pencarian</a>
                                @endif
                            </div>
                        @endif
                    </section>
                        ...
                        <div class="flex flex-wrap justify-center gap-6 md:gap-10">
                            ...
                            @foreach ($genres as $genre)
                                <a href="{{ route('home', ['search' => $genre['name']]) }}"
                                    class="flex flex-col items-center group w-20 md:w-24 cursor-pointer">
                                    <div
                                        class="w-16 h-16 md:w-20 md:h-20 bg-slate-800 rounded-2xl flex items-center justify-center text-2xl md:text-3xl mb-3
                                    ...
                                    <span
                                        class="text-sm font-medium text-gray-400 group-hover:text-white transition-colors">{{ $genre['name'] }}</span>
                                </a>
                            @endforeach
                        </div>
                    </section>
